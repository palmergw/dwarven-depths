...
docker run --rm \
  ...
  --workdir /work \
  "$IMAGE" \
  bash -c 'mkdir -p "$CARGO_HOME" "$CARGO_TARGET_DIR" .ddh/desktop-package && pnpm install --frozen-lockfile && pnpm --filter @dwarven-depths/desktop desktop:build && evidence_args=() && if [[ "$DD_CAPTURE_DESKTOP_EVIDENCE" == "1" ]]; then evidence_args=(docs/visual-evidence/release-closeout/wip-01/desktop); fi && xvfb-run --auto-servernum --server-args="-screen 0 1600x1000x24" dbus-run-session -- node scripts/smoke-desktop-runtime.mjs "$CARGO_TARGET_DIR/release/dwarven-depths-desktop" "${evidence_args[@]}" && rm -rf .ddh/desktop-package/* && cp "$CARGO_TARGET_DIR"/release/bundle/deb/*.deb .ddh/desktop-package/'
