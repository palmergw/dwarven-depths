# Layered Shuttergate tutorial-map proof of concept
...
**Product category:** tutorial map. The current scale is accepted only within that category and is not evidence that Dwarven Depths has solved large-map composition.
...
It proves presentation composability for a bounded tutorial court only. It does not claim large-map scale, runtime integration, simulation authority, HUD approval, final animation, or overall issue #286 approval.
...
3. `entrance-shell`
4. world subjects in front of route-crossing architecture
5. screen-space indicators
6. HUD

The complete clean plate is deterministically reconstructed from the same-camera environment base plus the entrance foreground pass. The rejected lower-edge framing is removed rather than retained as a decorative foreground artifact. With no subject inserted, reconstruction is byte-identical to the committed complete plate.
...
3. [`blender/outputs/production-sprite-traversal.png`](blender/outputs/production-sprite-traversal.png) — opacity-normalized 56 px Warden and 44 px raiders across the tutorial route.
4. [`evidence/shared-camera-foreground-isolation.png`](evidence/shared-camera-foreground-isolation.png) — the sole full-frame entrance RGBA artifact on checkerboard.
...
The entrance shell supplies the only route-local occlusion proof. Decorative lower-edge framing was removed because it added no useful gameplay or architectural information. Production sprites receive a bounded opacity gain at presentation time while preserving source canvas, pivot, nominal scale, and nonzero silhouette support.
