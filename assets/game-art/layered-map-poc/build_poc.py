...
BLENDER_STATIC_DEPTH=BLENDER_OUTPUTS/'static-scene-depth.bin'
...
def assert_authoring_reproducible()->None:
 ...
 if set(spatial)!={'schemaVersion','mapId','frame','world','camera','anchors','staticDepth','source'} or spatial['schemaVersion']!=2 or spatial['mapId']!='map.shuttergate_hall':
  raise ValueError('unexpected Shuttergate spatial contract')
 if set(spatial['staticDepth'])!={'asset','sha256','encoding','byteOrder','rowOrder','width','height','cameraDepthRange','noSurfaceCode','quantization','maximumQuantizationError'} or spatial['staticDepth']['sha256']!=sha(BLENDER_STATIC_DEPTH):
  raise ValueError('Shuttergate static depth contract drift')
