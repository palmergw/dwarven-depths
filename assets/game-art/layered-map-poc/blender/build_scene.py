...
APPROACH_SUBJECT_FOREGROUND_OBJECTS = {
    # Architecture that may sit camera-nearer than the hostile billboard. The
    # renderer also depth-clips this set against the authored hostile plane so
    # one object cannot redraw its camera-far pixels over the sprite.
...
OUTPUT_CONTRACT = {
    ...
    "entrance-route-ground-foreground.png": "straight-alpha-authored-ground-occluders-only",
...
def verify_render_reproducibility(manifest):
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        indexed_ground_output = output_root / "entrance-route-ground-foreground.png"
        render_indexed_foreground(indexed_ground_output, APPROACH_GROUND_OCCLUDER_OBJECTS)
        assert pixel_digest(indexed_ground_output) == pixel_digest(
            OUT / "entrance-route-ground-foreground.png"
        ), "stale indexed ground foreground pixels"
        indexed_output = output_root / "entrance-route-foreground.png"
        render_indexed_foreground(
            indexed_output,
            APPROACH_SUBJECT_FOREGROUND_OBJECTS,
            maximum_camera_depth=subject_camera_depth(),
        )
...
def subject_camera_depth():
    camera_space = bpy.context.scene.camera.matrix_world.inverted() @ Vector(APPROACH_SUBJECT_WORLD_POINT)
    return -camera_space.z


def render_indexed_foreground(output, object_names, maximum_camera_depth=None):
    ...
    alpha_output = id_mask.outputs["Alpha"]
    if maximum_camera_depth is not None:
        depth_test = nodes.new("CompositorNodeMath")
        depth_test.operation = "LESS_THAN"
        depth_test.inputs[1].default_value = maximum_camera_depth
        links.new(render_layers.outputs["Depth" if "Depth" in render_layers.outputs else "Z"], depth_test.inputs[0])
        depth_mask = nodes.new("CompositorNodeMath")
        depth_mask.operation = "MULTIPLY"
        links.new(alpha_output, depth_mask.inputs[0])
        links.new(depth_test.outputs[0], depth_mask.inputs[1])
        alpha_output = depth_mask.outputs[0]
    links.new(alpha_output, set_alpha.inputs["Alpha"])
...
render_indexed_foreground(OUT / "entrance-route-ground-foreground.png", APPROACH_GROUND_OCCLUDER_OBJECTS)
render_indexed_foreground(
    OUT / "entrance-route-foreground.png",
    APPROACH_SUBJECT_FOREGROUND_OBJECTS,
    maximum_camera_depth=subject_camera_depth(),
