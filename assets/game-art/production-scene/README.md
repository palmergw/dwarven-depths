...
- The painted route is represented as three pixel-surveyed visible depth zones. The upper gate has one architecture-hidden transition; the lower gate mouth remains visibly floored and is marked as a visible depth transition. The route terminates at the foreground-shutter backstop. There is no rail-crossing, winding-path, branching-path, or global exposed-floor-mask requirement.
- The diagnostic route overlay uses gold only on surveyed visible floor, cyan only across the upper architecture-hidden span, and green around the visible lower gate mouth. It is never player UI. The shutter is a nontraversable objective; the route terminates at its threshold.
...
| Route, entrance, gate | Pixel-surveyed piecewise topology, background entrance, one hidden upper transition, one visible lower gate mouth, terminal shutter objective | Local readability grading | Topology, entrance, portal, objective, or depth-zone change | #286 / product owner |
...
| Occlusion | Accept the two fixed #287 anchors and registered gate/depth transitions | Two off-route samples demonstrate partial architecture masking; the route board distinguishes the one hidden span from the visible gate mouth | Production portal masks, traversal, and movement are not claimed: #273 |
