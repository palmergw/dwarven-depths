...
Portal diagnostics are original deterministic Pillow outputs, not generated art. The builder fills the approved entity alpha footprints with green/magenta to create review-only solid proxies and draws 56/44 px alternating-band cards on the approved canvases and ground pivots. These assets are manifest-bound as diagnostics and never contribute to runtime reconstruction. The canonical `sources/foreground/portal-*-gate-mask.png` rasters were source-authored against the registered clean plate after the approved solid/card audit rejected generated profile, lintel, and jamb polygons. The builder contains no portal-mask geometry: it validates those binary `L` inputs, copies registered clean-plate pixels through them, and sweeps the diagnostic subjects at one- or two-pixel boundary increments. The upper wholly-hidden interval is encoded as a non-rendered portal state only after the incoming facade edge covers the subject and ends before the outgoing edge reveals it.
