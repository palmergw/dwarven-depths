...
Portal diagnostics are original deterministic Pillow outputs, not generated art. The builder fills the approved entity alpha footprints with green/magenta to create review-only solid proxies and draws 56/44 px alternating-band cards on the approved canvases and ground pivots. These assets are manifest-bound as diagnostics, never contribute to runtime reconstruction, and are swept at fixed route increments to audit the hand-authored foreground-only parapet/lintel/jamb masks. The upper wholly-hidden interval is encoded as a non-rendered portal state; it never copies sky or background pixels over a subject.
