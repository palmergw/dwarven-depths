...
The route walkable mask is a manually authored survey of visible floor boundaries in the clean plate. It is not generated by stroking or expanding the route polyline. Verification independently checks every route-segment sample against that survey and rejects HUD or foreground overlap. The survey is diagnostic contract data only and is never rendered in player mode.
