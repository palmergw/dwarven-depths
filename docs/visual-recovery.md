...
Issue #276 is captured by `pnpm capture:responsive-matrix` and the committed packet is checked by `pnpm verify:responsive-matrix`. Capture refuses tracked worktree changes and binds every image to the built client source head, the Shuttergate fixture where combat is shown, viewport, state, presentation settings, tick, entity count, target geometry, overflow results, and screenshot checksum; browser console or page errors abort capture. Publication verification rejects packet tampering and any product-code drift after the captured source head, allowing the evidence files themselves to be committed without falsely claiming that their self-referential commit hash existed when they were captured.
...
- Capture completed without browser console or page errors, and publication verification passes at the reviewed head.
