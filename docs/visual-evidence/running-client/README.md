...
The final hostile uses the exact shared-camera projection of Blender route point `(8.0,17.0,0.39)`, producing runtime anchor `(1110,253)`. The renderer now distinguishes ground-level occlusion from upright-subject occlusion: local rubble is drawn above rings/effects but below the hostile, while the route-facing architecture is depth-clipped against the hostile billboard plane and drawn afterward. The hostile remains over the native-renderer `entrance-route-rear.png` tunnel surface. No alternate coordinate, diagnostic probe, or synthetic composition is used as product evidence.
...
- Layer order: environment with rear architecture, world rings/effects, ground-level occluders, world subjects, depth-clipped entrance shell/route-facing architecture, screen-space focus indicator, HUD.
...
- Runtime and independent capture decoding must agree that the final hostile is above the rear witness and ground-level occluders, beneath camera-nearer architecture, and that the complete ring/effect footprints are clipped by the union of ground and architectural passes.
