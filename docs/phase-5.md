...
- a semantic results action that downloads deterministic schema-v2 authoritative replay evidence with a stable checksum-derived filename; `sim replay --client-evidence <file> --content <file> --scenario <file> --verify` binds its commands and terminal checksums to explicit shared content/scenario inputs and fails closed on malformed or tampered evidence.
