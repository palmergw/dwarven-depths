...
- an accessible authoritative-failure result that preserves the worker reason, receives focus, disposes the failed worker, rejects stale-worker messages, and returns by keyboard or mouse to a clean checkpoint without synthesizing terminal evidence.
